...
class SparseMSEEvaluator(MSEEvaluator):
    ...
    and some target sentence embedding for a sparse encoder.
    ...
    def __call__(self, model: SparseEncoder, output_path: str = None, epoch=-1, steps=-1) -> dict[str, float]:
        if epoch != -1:
            if steps == -1:
        ...
        # Using euclidean_sim function to calculate similarity
        similarity = pairwise_euclidean_sim(self.source_embeddings, target_embeddings)
        # Convert similarity (negative distance) to MSE and scale by 100
        # Since similarity = -distance, and MSE = distance²/dimensions
        # We need to square the negative of similarity
        mse = ((-similarity) ** 2).mean()
        mse *= 100
