...
def plot_grad_flow(named_parameters: iter) -> plt.figure:
    """
    source: https://github.com/MLAlex1/bert_tensorboard/blob/42b8e58c3d4465f0df51c4c347050904dc0a5696/utils_bert.py#L12
    ...
    for (n, p) in named_parameters:
        if p.requires_grad and p.grad is not None:
            name_layers.append(
                '.'.join([name_replace.get(el, el) for el in n.split(".") if el not in ['weight', 'bert', 'self']]))
    ...
    sns.barplot(x=name_layers, y=mean_grads, palette=['r'] * len(name_layers), alpha=0.9, ax=ax[2],
                label="mean grads", )

    sns.barplot(x=name_layers, y=zero_grads, palette=['k'] * len(name_layers), alpha=0.9, ax=ax[0],
                label="percentage zero grads")

    sns.barplot(x=name_layers, y=stds, palette=['c'] * len(name_layers), alpha=0.9, ax=ax[1],
                label="standard dev grads")
