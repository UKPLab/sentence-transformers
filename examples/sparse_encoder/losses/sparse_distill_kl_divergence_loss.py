...
def compute_labels(batch):
    ...
    sim1 = teacher_model.similarity_pairwise(emb_queries, emb_passages1)
    sim2 = teacher_model.similarity_pairwise(emb_queries, emb_passages2)

    labels = torch.stack([sim1, sim2], dim=1)

    return {"label": labels.tolist()}


train_dataset = train_dataset.map(compute_labels, batched=True)
loss = SparseDistillKLDivLoss(student_model, similarity_fct=teacher_model.similarity_pairwise)
